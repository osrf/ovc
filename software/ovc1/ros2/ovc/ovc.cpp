...
OVC::OVC()
: init_complete(false), fd(-1), dma_buf(NULL)
{
}
...
bool OVC::wait_for_image(uint8_t **p)
{
...
bool OVC::set_exposure(float seconds)
{
  ...
  exposure.exposure_usec = seconds * 1000000;
  int rc = ioctl(fd, OVC_IOCTL_SET_EXPOSURE, &exposure);
  if (rc) {
  ...
  const uint8_t *img, const uint32_t len, const uint32_t *sig)
{
  ...
  bool ok = *sig == crc;
  if (!ok)
    printf("mismatch.  rx: 0x%08x   calc: 0x%08x\n", (unsigned)*sig, crc);
  return ok;
}
